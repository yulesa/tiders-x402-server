...
impl PyPriceTag {
    ...
    ///     token (TokenDeployment): Token with decimals and EIP712 information, currently only USDC is supported.
    ...
    fn new(
        pay_to: &str,
        amount_per_item: PyObject,
        token: &PyUSDCDeployment,
        min_total_amount: Option<PyObject>,
        ...
        let token: TokenDeployment = (&token.inner).into();
        ...
        let amount_per_item = if let Ok(amount_str) = amount_per_item.extract::<String>(py) {
            // Parse as string (MoneyAmount) first, then convert to TokenAmount
            let money_amount = MoneyAmount::from_str(&amount_str)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
            money_amount.as_token_amount(token.decimals as u32)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?
            ...
            TokenAmount::from(U256::from(amount_int as u64))
        } else {
        ...
        let min_total_amount = if let Some(min_obj) = min_total_amount {
            if let Ok(min_str) = min_obj.extract::<String>(py) {
                ...
                let min_money = MoneyAmount::from_str(&min_str)
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
                Some(min_money.as_token_amount(token.decimals as u32)
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?)
                ...
                Some(TokenAmount::from(U256::from(min_int as u64)))
            } else {
                return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
...
impl PyUSDCDeployment {
    ...
    ///
    /// Args:
    ///     network (str or Network): Network name or enum value.
    ///
    /// Returns:
    ///     USDCDeployment: The deployment for the network.
    #[staticmethod]
    fn by_network(py: Python, network: PyObject) -> PyResult<Self> {
        ...
        let network = match network_str.as_str() {
            ...
            "base" => Network::Base,
            "xdc_mainnet" => Network::XdcMainnet,
            "avalanche_fuji" => Network::AvalancheFuji,
            "avalanche" => Network::Avalanche,
            _ => return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>("Invalid network")),
        };

        Ok(Self {
            inner: USDCDeployment::by_network(network).clone(),
        })
    }
}
