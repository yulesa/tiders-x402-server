...
impl ClickHouseDatabase {
    ...
    /// Uses `DESCRIBE TABLE` to get column names and types, then maps to Arrow fields.
...
/// Helper function that Maps a ClickHouse type string to its Arrow equivalent.
///
/// Used by `get_table_schema_internal` which parses `DESCRIBE TABLE` output.
...
/// Helper function that parses ClickHouse Decimal types into Arrow Decimal128.
...
/// Helper function that extracts a single numeric parameter from a ClickHouse type string, e.g. "Decimal64(4)" → 4.
...
/// Helper function that extracts two numeric parameters from a ClickHouse type string, e.g. "Decimal(20, 4)" → (20, 4).
...
/// Helper function that reads Arrow IPC stream bytes into a vector of `RecordBatch`es.
